...
{
    public class UserManagementGetWorkerByCriteria : IScenario
    {
        public void Start()
        {
            ...
            using (var proxy = new UserManagementProxy(remoteHost))
            {
                var result = proxy.ExecuteReadOperation(
                    new GetWorkersByCriteriaOperation
                    {
                        OperatorGroupId = 1
                    }
                    );
        ...
        public UserManagementGetWorkerByCriteria(IController controller, IUserManagementSettings settings)
        {
